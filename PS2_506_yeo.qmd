...
random_walk <- function(n) {
  ...
  for (i in 1:n) {
    ...
    position <- position + step_for
  }
  
  return(position)
}
#not getting the same values as in the problem set? due to randomness?
random_walk(10)
random_walk(10)
```
...
Using the loop which is what I did above so just copy/pasting.

```{r}
#| echo: true
#loop is what we just did so just using the same code w/ diff name
...
Using the built in vectorized functions in R. I used this <https://stackoverflow.com/questions/42453152/multiple-conditional-statements-in-square-brackets> to help me come up with how to use square brackets with conditionals, kinda like we did for the first problem set with the data frame. 
...
random_walk2 <- function(n) {
  #order doesn't matter, can replace, have to do n time
  set.seed(23)
  random_step <- runif(n)
  step_for <- sample(c(-1, 1), size = n, replace = TRUE)
  #can't use if statements, bc doing multiple values?
  #same uniform conditions as the loop, conditional check with square bracket 
  step_for[step_for == 1 & random_step < 0.05]  <- 10
  step_for[step_for == 1 & random_step >= 0.05]  <- 1
  step_for[step_for == -1 & random_step < 0.20] <- -3
  step_for[step_for == -1 & random_step >= 0.20]  <- -1
  #add up all the steps in the walk to get where the final position is
  sum(step_for)
}
#
random_walk2(10)
...
#should use lapply - easiest to work with from notes
random_walk3 <- function(n) {
  set.seed(23)
  rand <- runif(n)
  step_for <- lapply(1:n, function(i) {
    #lapply is basically our loop so back to sample from 1
    num_step <- sample(c(-1,1), size = 1)
    #conditonals still same so can use same if statements as loop, just copy 
    if (num_step == 1) {
      if (rand[i] < 0.05) {
        num_step <- 10
      } else {
  ...
  #lapply turns things into a list but we want a vector so have to unlist it
  #lapply func all in the step_for variable so we want this to be unlisted
  steps <- unlist(step_for)
  #add up steps to get final position like in the second random walk func
  sum(steps)
}
random_walk3(10)
random_walk3(1000)
...
This had the unlist function then I looked it up more after <https://www.mjandrews.org/notes/functionals/>. 
...
```{r}
#| echo: true
#add a seed in this part to control for random
random_walk1(10)
random_walk2(10)
random_walk3(10)
random_walk1(1000)
...
```
For $n=1000$, we first note the unit for this case is microseconds and with this it is shown that the vectorized function is the fastest then the for loop and lastly the lapply function. For the case when $n=100000$, the unit changes to milliseconds and took much longer to run than the first case. Again we see that the vectorized is the fastest, then the for loop, and lastly the lapply function. This matches what we discussed in class about run times with different versions of functions.
...
set.seed(23) 
sim <- 10000
prob_zero <- function(n){
  zero_end <- replicate(sim, random_walk(n))  
  mean(zero_end == 0)  
}
...
prob_zero(1000)
```
The probabilities that the random walk ends at 0 if the number of steps decreases as the sample size increases. This can be seen from the code above, which shows a simulation for the different $n$ values. The probability for ending at $0$ for $n=10$ is $.131$. The probability for ending at $0$ for $n=100$ is $.0215$. The probability is practically $0$ for $n=1000$, with probability $.006$. This happens because each additional step introduces more variability and the jumps in step size we make with the small probabilities make it increasingly unlikely that the sum of steps will exactly cancel out to 0.
...
n <- 100000
#realized this is just one hours rather than the eight I thought it was 
  #so I have to use matrices
midnight_to_seven_matrix <- matrix(rpois(n * 8, lambda = 1), 
                                   nrow = n, ncol = 8)
midnight_to_seven_day <- rowSums(midnight_to_seven_matrix)

#rush hour is just two hours, 8 and 17, not continuous in this time range
rush_hour_matrix <- matrix(rnorm(n * 2, mean = 60, sd = sqrt(12)), 
                           nrow = n, ncol = 2)
...
nine_to_four_matrix <- matrix(rpois(n * 8, lambda = 8), 
                              nrow = n, ncol = 8)
nine_to_four_day <- rowSums(nine_to_four_matrix)

six_to_eleven_matrix <- matrix(rpois(n * 6, lambda = 12), 
                               nrow = n, ncol = 6)
...
print(mean(total_car_pass))

```

The average number of cars that pass by this intersection per day is 264 cars, rounding down from the value we found of $264.0004$. 
...
In order to see if these variables are appropriate to use in a linear regression model we want to look at the distribution or density of values in that variable and see if it looks roughly normal, which comes from the fact that the linear regression analysis requires all variables to be multivariate normal and the easiest way to check this is using a histogram or QQ plot (<https://www.statisticssolutions.com/free-resources/directory-of-statistical-analyses/assumptions-of-linear-regression/>). I will be using histograms because I think they are easier to make and deal with. If the data does not follow a normal distribution, a non-linear transformation (e.g., log-transformation) can fix the issue.
...
beta_hat_1
```
This matches the output we got for the linear regression model for view count, just has more decimal places in this case. I had issues with all the variables being NA, which came from NAs still being present in our predictors so had to drop these (this is done automatically in lm so we have to do it manually). 
