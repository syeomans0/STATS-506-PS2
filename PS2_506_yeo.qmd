...
```
From the histograms we can see this distribution for `{r} view_count`, `{r} like_count`, `{r} dislike_count`, and `{r} comment_count` are all heavily right skewed with some potential outliers at very high values along the x-axis. Since these are heavily skewed we should use a log transformation to deal with the extreme values. After this log transformation we should hopefully see something that looks a bit better and more Normal. The histogram for the `{r} favorite_count` is the only odd one out from this group of variables. This distribution looks to be...

The histogram with the log transformation looks very symmetric and bell-shaped so we should now be able to use it for linear regression. We have to make sure this transformation stays with this going forward so we have to ...
