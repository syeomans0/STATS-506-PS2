...
```
For like count, there is only one statistically significant variable which is year, not any of the binary flags for the characteristics for the ads. In context this means, ... The danger flag has a p-value of $0.0886$, this doesn't quite meet the $0.05$ significance cut off but it would be significant if we were testing it at the $0.1$ cut off, so I will discuss this too. In context, this means that ...
...
```
For the dislike count, just like the like count, the only significant result is year, if the cutoff is $0.05$. If we went up to the $0.1$ cutoff, patriotic would become significant as well. 
...
```
For the comment count, there are not any significant results at the $0.05$ cut off but there are some at the $0.1$ cut off. The variables significant at the $0.1$ level are patriotic and year. 
